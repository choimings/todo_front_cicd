...
const ItemPanel = ({ pageTitle, filterCompleted, filterImportant }) => {
  ...
  useEffect(() => {
    ...
    const fetchGetItems = async () => {
      try {
        setLoading(true);
        await dispatch(fetchGetItemsData(userKey)).unwrap(); // useEffect 내부에서 dispatch 함수를 호출할 때 async/await을 사용할 수 없을 때 unwrap()을 사용
      } catch (error) {
  ...
  return (
    <div className="panel bg-[#212121] w-4/5 h-full rounded-sm border border-gray-500 py-5 px-4 overflow-auto">
      {userKey ? (
        ...
        <div className="w-full h-full flex items-center justify-center">
          <button className="flex justify-center items-center gap-2 bg-gray-300 text-black py-2 px-4 rounded-md  font-semibold w-fit">
